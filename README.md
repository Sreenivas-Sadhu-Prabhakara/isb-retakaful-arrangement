...
| POST | `/v1/retakaful-treaty-management-plan/initiate` | Initiate |
| GET | `/v1/retakaful-treaty-management-plan` | Retrieve (list) |
| GET | `/v1/retakaful-treaty-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/retakaful-treaty-management-plan/{crId}/update` | Update |
| PUT | `/v1/retakaful-treaty-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/retakaful-treaty-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-retakaful-arrangement:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-retakaful-arrangement ./helm -n isb-operations
```

Exposed through the platform Gateway at path prefix `/isb-retakaful-arrangement` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
